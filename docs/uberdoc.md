<!DOCTYPE html>
<html><head><meta charset="utf-8" content="text/html" http-equiv="Content-Type" /><meta content="FIXME: write description" name="description" /><style type="text/css">/**
 * SyntaxHighlighter
...
</script><title>tracer -- Marginalia</title></head><body><table><tr><td class="docs"><div class="header"><h1 class="project-name"><a href="http://example.com/FIXME">tracer</a></h1><h2 class="project-version">0.1.0-SNAPSHOT</h2><br /><p>FIXME: write description</p>
</div><div class="dependencies"><h3>dependencies</h3><table><tr><td class="dep-name">org.clojure/clojure</td><td class="dotted"><hr /></td><td class="dep-version">1.6.0</td></tr><tr><td class="dep-name">thunknyc/richelieu</td><td class="dotted"><hr /></td><td class="dep-version">0.4.0</td></tr></table></div></td><td class="codes" style="text-align: center; vertical-align: middle;color: #666;padding-right:20px"><br /><br /><br />(this space intentionally left almost blank)</td></tr><tr><td class="docs"><div class="toc"><a name="toc"><h3>namespaces</h3></a><ul><li><a href="#tracer.core">tracer.core</a></li></ul></div></td><td class="codes">&nbsp;</td></tr><tr><td class="docs"><div class="docs-header"><a class="anchor" href="#tracer.core" name="tracer.core"><h1 class="project-name">tracer.core</h1><a class="toc-link" href="#toc">toc</a></a></div></td><td class="codes" /></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(ns tracer.core
  (:require [richelieu.core :as richelieu]
            [clojure.pprint :as pprint]))</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(def ^:dynamic *trace-level* 0)</pre></td></tr><tr><td class="docs"><p>FIXME -- doesn't work</p>
</td><td class="codes"><pre class="brush: clojure">(def ^:dynamic *trace-stream* #'*out*)</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(def trace-indent-per-level 2)</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(def indent-character \space)</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(defn- trace-indent []
  (* trace-indent-per-level *trace-level*))</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(defonce traced-vars (atom '{}))</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(defn traced? [f]
  (some #{f} (keys @traced-vars)))</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(defn untrace [f]
  (richelieu/unadvise-var f (@traced-vars f))
  (println &quot;Untracing&quot; f)
  (swap! traced-vars dissoc f))</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(defn untrace-ns [ns]
  (doseq [traced-fn (filter #(= (:ns (meta %)) ns) @traced-vars)]
    (untrace traced-fn)))</pre></td></tr><tr><td class="docs"><p>Prints a nicely-formatted list of the currently-traced function
  with its <code>args</code>, indented based on the current <code>*trace-level*</code>.</p>
</td><td class="codes"><pre class="brush: clojure">(defn report-before
  [args]
  (pprint/cl-format true &quot;~&amp;~vt~d: (~s ~{~s~^ ~})~%&quot; (trace-indent) *trace-level* richelieu/*current-advised* args))</pre></td></tr><tr><td class="docs"><p>Prints a nicely-formatted list of the currently-traced function
  with its <code>retval</code>, indented based on the current <code>*trace-level*</code>.</p>
</td><td class="codes"><pre class="brush: clojure">(defn report-after
  [retval]
  (pprint/cl-format true &quot;~&amp;~vt~d: ~s returned ~s~%&quot; (trace-indent) *trace-level* richelieu/*current-advised* retval))</pre></td></tr><tr><td class="docs">
</td><td class="codes"><pre class="brush: clojure">(defn- get-wrapped-fn [f when-fn report-before-fn report-after-fn]
    ...
    (if when-fn
      (fn [f &amp; args]
        (if (apply when-fn args)
          ...
          (apply f args)))
      trace-report-fn)))</pre></td></tr><tr><td class="docs"><p>Turns on tracing for the function bound to F, a var.</p>

<p>  If <code>when-fn</code> is provided, the trace reporting described below will
  only occur when <code>when-fn</code> returns a truthy value when called with
  the same args as the traced functions. If <code>when-fn</code> is not provided,
  every call to the traced function will be reported.</p>

<p>  If <code>report-before-fn</code> is provided, it will be called before the
  ...
  <code>tracer.core/report-before</code> if not provided.</p>

<p>  If <code>report-after-fn</code> is provided, it will be called after the traced
  function is called, with that function's return value as its
  argument, and should print some useful output. It defaults to
  <code>trace.core/report-after</code> if not provided.</p>
</td><td class="codes"><pre class="brush: clojure">(defn trace
  ...
  (let [advice-fn (get-wrapped-fn f when-fn report-before-fn report-after-fn)]
    ...
    (swap! traced-vars assoc f advice-fn)))</pre></td></tr><tr><td class="spacer docs">&nbsp;</td><td class="codes" /></tr></table><div class="footer">Generated by <a href="https://github.com/gdeer81/marginalia">Marginalia</a>.&nbsp;&nbsp;Syntax highlighting provided by Alex Gorbatchev's <a href="http://alexgorbatchev.com/SyntaxHighlighter/">SyntaxHighlighter</a><div id="floating-toc"><ul><li class="floating-toc-li" id="floating-toc_tracer.core">tracer.core</li></ul></div></div><script type="text/javascript">SyntaxHighlighter.defaults['gutter'] = false;
